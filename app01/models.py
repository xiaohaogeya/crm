...
class Customer(models.Model):
    ...
    introduce_from = models.ForeignKey('self', verbose_name="转介绍自学员", blank=True, null=True,on_delete=models.CASCADE)  #self指的就是自己这个表，和下面写法是一样的效果,自关联
    ...
    #customers替换的就是反向查询时,小写表名_set


    # ret = models.Customer.objects.get(id=1)
    # ret.consultant.username
    # ret = models.Userinfo.objects.get(id=2)
    # ret.customers.all()
    # models.Userinfo.objects.values('customers__name')
...
class ClassList(models.Model):
    ...
    semester = models.IntegerField("学期") #北京校区python脱产26期等
